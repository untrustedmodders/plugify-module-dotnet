...
public class ManifestFileGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor DuplicateExport = new(
        id: "PLG002",
        title: "Duplicate exported method",
        messageFormat: "Plugify manifest not written: {0}",
        category: "Plugify",
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true);

    private static readonly DiagnosticDescriptor NoExportedMethods = new(
        id: "PLG003",
        title: "No exported methods found",
        messageFormat: "No methods with [NativeExport] found in '{0}', so no Plugify manifest was written",
        category: "Plugify",
    ...
    private static void GenerateManifestClass(SourceProductionContext context, Compilation compilation, ImmutableArray<ExportedMethod?> methods)
    {
        ...
        // Reported rather than emitted as a type: a diagnostic reaches the build
        // log and the IDE error list, whereas a generated marker class only
        // showed up for someone already reading generated sources.
        if (validMethods.Count == 0)
        {
            context.ReportDiagnostic(Diagnostic.Create(NoExportedMethods, Location.None, assemblyName));
            return;
        }
        ...
        // Plugify refuses a manifest with a repeated method name or symbol at
        // load, which is a long way from the attribute that caused it.
        var duplicates = manifestMethods
            .GroupBy(m => m.Name, StringComparer.Ordinal)
            .Where(g => g.Count() > 1)
            .Select(g => $"method '{g.Key}' is exported more than once")
            .Concat(manifestMethods
                .GroupBy(m => m.FuncName, StringComparer.Ordinal)
                .Where(g => g.Count() > 1)
                .Select(g => $"symbol '{g.Key}' is generated more than once"))
        ...
        if (duplicates.Count > 0)
        {
            context.ReportDiagnostic(
                Diagnostic.Create(DuplicateExport, Location.None, string.Join("; ", duplicates)));
            return;
        }
